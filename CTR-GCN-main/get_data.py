...
#     """
#     try:
#         x_train = np.load('./data/train_bone.npy')
#         x_test = np.load('./data/test_A_bone.npy')
#         y_train = np.load('./data/train_label.npy')
#         y_test = np.load('./data/test_A_label.npy')
...
def remove_zero_samples_and_augment(skes_joints: np.ndarray, labels: np.ndarray, target_frames: int = 300) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for n in range(N):
        ...
        valid_data = skes_joints[n]  # 不再筛选，只保留原始数据

        # 处理有效帧数不足的情况
        if valid_frame_count < target_frames:
            augmented_data = np.zeros((valid_data.shape[0], target_frames, valid_data.shape[2], valid_data.shape[3]))
            # 将有效帧数放到中间，其他帧数使用0填充
            start_idx = (target_frames - valid_frame_count) // 2
            augmented_data[:, start_idx:start_idx + valid_frame_count, :, :] = valid_data[:, valid_frames]
        else:
            # 如果帧数足够，直接使用
            augmented_data = valid_data[:, :target_frames, :, :]
